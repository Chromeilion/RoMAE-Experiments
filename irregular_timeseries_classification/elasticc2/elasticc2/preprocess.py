...
from elasticc2.datatools.get_fits_elasticc_data import GetElasticcData
from elasticc2.datatools.get_lc_md import processing
from elasticc2.datatools.get_partitions import *
from elasticc2.datatools.get_qt_feat_and_h5_file import *
...
#    check_directory('{}/FULL_ELASTICC_TRAIN.tar'.format(path_save_fits))
    ...
    print(
        'Generating data files joined and getting QT from static features (metadata) ...')
    if use_paper_partitions == True and os.path.exists(
            './data_partition_paper/partitions.parquet'):
    ...
    total_partitions_not_blind = open_partitions(path_blind_partitions,
                                                 path_partitions,
                                                 path_save_final_data)
    all_partitions, pd_final_v2, md_norm_list = get_qt_metadata(pd_final,
    ...
    create_dataset(all_partitions, pd_final_v2, md_norm_list, num_folds,
                   path_save_final_data)
    ...
    if os.path.exists('./dynamic_features/features_20220817_2048.parquet'):
        ...
        print(
            'Generating QT for dynamic features in different times [1, 2, 4, 8, 16, 32, ..., 1024, 2048] ...')
        path_dataset = '{}/elasticc_final.h5'.format(path_save_final_data)
        h5_file = h5py.File(path_dataset)
        ...
        for eval_time in times_to_eval:
            for fold in range(num_folds):
                print(
                    '- creating QT for dynamic features in {} days and fold {}'.format(
                        eval_time, fold))
                these_idx = h5_file.get('training_{}'.format(fold))[:]
                norm_add_feat_col = torch.from_numpy(
                    h5_file.get('norm_add_feat_col_{}'.format(eval_time))[:][
                        these_idx])
                create_qt(norm_add_feat_col,
                          'fe_{}_fold_{}'.format(eval_time, fold),
                          path_save_final_data)
