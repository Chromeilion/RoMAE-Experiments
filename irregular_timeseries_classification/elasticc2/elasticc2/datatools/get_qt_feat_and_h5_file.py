...
def open_partitions(path_blind_partitions, path_partitions,
    ...
    mapping_to_int = {key: idx - 1 for idx, key in
                      enumerate(total_partitions.label.unique())}
    print('- ', total_partitions.label.unique())
    print('- ', mapping_to_int)

    with open("./{}/dict_classes.json".format(path_save_final_data),
              "w") as outfile:
    ...
    total_partitions_not_blind['label_int'] = total_partitions_not_blind.apply(
        lambda x: apply_mapping(x['label']), axis=1)

    return total_partitions_not_blind


def ordered_partitions(df, total_partitions, fold):
    this_partition = total_partitions[
        (total_partitions['partition'] == 'training_%d' % fold) | \
...
def create_dataset(all_partitions, all_data, md_norm_list, num_folds,
                   path_save_final_data):
    with h5py.File('{}/elasticc_final.h5'.format(path_save_final_data),
                   'w') as hf:
        hf.create_dataset("data", data=np.stack(all_data['data'].to_list(), 0))
        hf.create_dataset("data-var",
                          data=np.stack(all_data['data_var'].to_list(), 0))
        ...
        hf.create_dataset("labels",
                          data=np.stack(all_data['label_int'].to_list(), 0))
        hf.create_dataset("mask_alert",
                          data=np.stack(all_data['mask_alert'].to_list(), 0))
        hf.create_dataset("time_alert",
                          data=np.stack(all_data['time_alert'].to_list(), 0))
        hf.create_dataset("time_phot",
                          data=np.stack(all_data['time_phot'].to_list(), 0))
        hf.create_dataset("mask_detection",
                          data=np.stack(all_data['mask_detection'].to_list(),
                                        0))
        hf.create_dataset("norm_feat_col",
                          data=all_data[md_norm_list].to_numpy())
        ...
        for fold in range(num_folds):
            ...
            hf.create_dataset("validation_%d" % fold,
                              data=aux_pd[aux_pd[
                                              'partition'] == 'validation_%d' % fold][
                                  'unique_id'].to_numpy())

        hf.create_dataset("test",
                          data=aux_pd[aux_pd['partition'] == 'test'][
                              'unique_id'].to_numpy())
        hf.close()
...
def get_qt_metadata(pd_final, total_partitions_not_blind, num_folds,
                    path_save_final_data):
    all_partitions = {}
    for fold in range(num_folds):
    ...
    for col in feat_list:
        ...
        pd_final_v2[new_name] = pd_final_v2[new_name].replace(
            [np.inf, -np.inf], -9999)
    ...
    for fold in range(num_folds):
        ...
        aux_pd = all_partitions['fold_%s' % fold]
        aux_idx = aux_pd[aux_pd['partition'] == 'training_%s' % fold][
            'unique_id'].to_numpy()
        create_qt(pd_final_v2[aux_feat_list].iloc[aux_idx], name_used,
                  path_save_final_data)
...
def add_dynamic_features(All_SNID, times_to_eval, path_dataset):
    ...
    aux_feat = \
        pd.read_parquet((
                                    './dynamic_features/features_20220817_%4d.parquet' % eval_time).replace(
            ' ', '0'))
    norm_name_used = 'norm'
    fe_norm_list = ['%s_%s' % (norm_name_used, feat) for feat in
                    aux_feat.columns]
    del aux_feat
    ...
    for eval_time in times_to_eval:
        ...
        for col in aux_feat.columns:
            ...
            ##### Deleting nans #######
            if np.isnan(aux.mean()):
                aux_feat['%s_%s' % (norm_name_used, col)] = aux_feat[
                    col].fillna(-9999)
            else:
                aux_feat['%s_%s' % (norm_name_used, col)] = aux_feat[col]
            aux_feat['%s_%s' % (norm_name_used, col)] = aux_feat[
                '%s_%s' % (norm_name_used, col)].replace([np.inf, -np.inf],
                                                         -9999)
        ...
        with h5py.File('./{}'.format(path_dataset), 'a') as hf:
            hf.create_dataset("norm_add_feat_col_%s" % eval_time,
                              data=all_features)
        hf.close()
