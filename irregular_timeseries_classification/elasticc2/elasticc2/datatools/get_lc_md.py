...
def pad_list(lc, nepochs):
    ...
    if pad_num >= 0:
        ...
        return np.array(lc)[
            np.linspace(0, nepochs - 1, num=MAX_EPOCHS).astype(int)]
...
def separate_by_filter(feat_time_series, bands):
    ...
    colors = {'u': 'b', 'g': 'g', 'r': 'r', 'i': 'orange', 'z': 'brown',
              'Y': 'k'}
    final_array = []
...
def mask_alert(time, mask, photflags):
    time_flatten = time.reshape(-1, )[
        (photflags.reshape(-1, ) != 0) & (photflags.reshape(-1, ) != 1024)]
...
def processing(base_dir, target_dir):
    for fine_class in tqdm(os.listdir(base_dir)):
        ...
        df = pd.read_pickle(
            '%s/lc_%s.pkl' % (os.path.join(base_dir, fine_class), fine_class))
        ...
        dict_transform = {'MJD': 'time', 'FLUXCAL': 'data',
                          'FLUXCALERR': 'data_var', 'PHOTFLAG': 'photflag'}
        ...
        for key_used in list_time_feat:
            pd_dataset[dict_transform[key_used]] = \
                df_grouped.apply(
                    lambda x: separate_by_filter(x[key_used], x[band_key]),
                    axis=1)
        pd_dataset['time'] = pd_dataset.apply(
            lambda x: normalizing_time(x['time']), axis=1)
        pd_dataset['mask'] = pd_dataset.apply(lambda x: create_mask(x['data']),
                                              axis=1)

        print("- masking")
        pd_dataset['mask_alert'] = pd_dataset.apply(
            lambda x: mask_alert(x['time'], x['mask'], x['photflag']), axis=1)
        pd_dataset['time_alert'] = pd_dataset.apply(
            lambda x: normalizing_time_alert(x['time'], x['mask_alert'],
                                             x['photflag']), axis=1)
        pd_dataset['time_phot'] = pd_dataset.apply(
            lambda x: normalizing_time_phot(x['time'], x['mask_alert']),
            axis=1)
        pd_dataset['mask_detection'] = pd_dataset.apply(
            lambda x: mask_detection(x['photflag']), axis=1)

        ###### Metadata by astronomical object ######
        df_aux = pd.read_pickle('%s/header_%s.pkl' % (
        os.path.join(base_dir, fine_class), fine_class))
