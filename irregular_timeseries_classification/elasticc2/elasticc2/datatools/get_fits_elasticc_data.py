...
class GetElasticcData(object):
    ...
    def run(self, multiprocess, num_cores=8):
        ...
        if self.toy_example:
            if len(self.all_object_files) < num_cores: num_cores = len(
                self.all_object_files)
            print('- we are using {} cores'.format(num_cores))

            with multiprocessing.Pool(processes=num_cores) as pool:
                _ = list(tqdm.tqdm(
                    pool.imap_unordered(self.processing_fits_by_class,
                                        self.all_object_files),
            ...
            for object_file in self.all_object_files:
                ...
                path_save_lc = './{}/lc_{}.pkl'.format(self.save_path,
                                                       name_object)
                path_save_header = './{}/header_{}.pkl'.format(self.save_path,
                                                               name_object)

                if multiprocess:
                    if len(all_cores_files) < num_cores: num_cores = len(
                        all_cores_files)
                    print('- we are using {} cores'.format(num_cores))

                    with multiprocessing.Pool(processes=num_cores) as pool:
                        mapped_values = list(tqdm.tqdm(
                            pool.imap_unordered(self.processing_fits,
                                                all_cores_files),
                        ...
                        df_cores_lc = pd.concat(df_cores_lc,
                                                axis=0).reset_index(drop=True)
                        df_cores_header = pd.concat(df_cores_header,
                                                    axis=0).reset_index(
                            drop=True)
                    ...
                    df_cores_lc = pd.concat(df_cores_lc, axis=0).reset_index(
                        drop=True)
                    df_cores_header = pd.concat(df_cores_header,
                                                axis=0).reset_index(drop=True)
    ...
    def processing_fits(self, core_file):
        ...
        * Core corresponds to a HEAD and PHOT file.
        ...
        for idx_sn in range(data_core_head.shape[0]):
            ...
            lightcurve = pd.DataFrame([list(row) for row in data_core_phot[
                                                            ptrobs_min - 1:ptrobs_max]],
        ...
        df_object_header = pd.concat(list_core_header, axis=0).reset_index(
            drop=True)
    ...
    def processing_fits_by_class(self, object_file):
        ...
        * Core corresponds to a HEAD and PHOT file.
        ...
        name_object = object_file.replace(
            '{}_{}_'.format(self.dataset_name, self.subset_name), "")
        ...
        if name_object in self.which_classes:
            files_path = './{}/{}/{}_{}_NONIaMODEL0*'.format(
                self.load_parent_file,
                object_file,
                self.dataset_name,
        ...
        try:
            ...
            print(
                "\n- object {} has a exception in its name file that you do not consider".format(
                    name_object))
            exit()
        ...
        for core_file in all_cores_files:
            ...
            df_object_lc.append(
                pd.concat(list_core_lc, axis=0).reset_index(drop=True))
            df_object_header.append(
                pd.concat(list_core_header, axis=0).reset_index(drop=True))

        df_cores_lc = pd.concat(df_object_lc, axis=0).reset_index(drop=True)
        df_cores_header = pd.concat(df_object_header, axis=0).reset_index(
            drop=True)
    ...
    def __get_cores(self, object_file):
        ...
        print(
            '- DATASET {} - PREPROCESSING OBJECT: {}'.format(self.dataset_name,
                                                             name_object))
        # Files of one class
