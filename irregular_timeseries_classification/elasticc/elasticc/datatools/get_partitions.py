...
def open_original_files(base_dir):
    ...
    for fine_class in tqdm(os.listdir(base_dir)):
        try:
            class_lightcurves = pd.read_pickle(
                os.path.join(base_dir, fine_class, f'lc_{fine_class}.pkl'))
    ...
    assert len(fine_class_grouping.keys()) == len(
        snid_per_fine_class.keys()), msg
...
def create_df_ids_labels(snid_per_fine_class):
    ...
    for fine_class in fine_class_grouping.keys():
        ...
        labels.append(
            [fine_class_grouping[fine_class]] * len(fine_class_snids))
...
def create_split(snids_df, sample_test, num_folds):
    ...
    for i, indexes in enumerate(
            kf.split(np.zeros(n_samples), training_validation.label)):
...
def check_partitions(snids_df, partitions, test_set, sample_test, num_folds):
    ...
    print('- ', (
                len(snids_df) - n_classes * sample_test) * num_folds + n_classes * sample_test)
    print('-', len(partitions))

    snids_df[snids_df['label'].isin(['Delta Scuti', 'Cepheid'])][
        'SNID'].astype(float).sort_values()

    for astroclass in snids_df['label'].unique():
        print('- ', astroclass.ljust(15),
              snids_df[snids_df['label'] == astroclass]['SNID'].astype(
                  float).mean())
    ...
    plt.hist(
        snids_df[snids_df['label'].isin(['Delta Scuti', 'Cepheid'])]['SNID'],
        label='dsct+ceph', density=False, alpha=0.8)
    plt.xscale('log')
    plt.yscale('log')
    plt.show()

    print('- ', snids_df[snids_df['SNID'].duplicated(keep=False)].sort_values(
        'SNID').iloc[:30])

    assert (
                       len(snids_df) - n_classes * sample_test) * num_folds + n_classes * sample_test == len(
        partitions)

    test_snids = partitions[partitions['partition'] == 'test']['SNID'].values
    non_test_snids = partitions[partitions['partition'] != 'test'][
        'SNID'].unique()
...
def save_partitions(base_dir, partitions, blind_partitions):
    ...
    blind_partitions.to_parquet(
        './{}/blind_partitions.parquet'.format(base_dir))
